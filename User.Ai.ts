...
import * as CoreClass from "./Core.Class";
import * as CoreInit from "./Core.Init";
import * as AiCore from "./Core"
import * as fs from "fs";
var ORTSData:ORTSData = {} as ORTSData;
var ORTSGameObjectPointer:Map<Number,CoreClass.GameObject> = new Map();

interface ORTSData{
    Player:Map<Number,CoreClass.GamePlayer>
}
...
AiCore.AddListener((AiConnect,AiMessage)=>{
    ...
    if(Command == "查看"){
        ...
        var UnitStatus = (()=>{ if(ORTSGameObjectPointer.get(AiMessage.user_id).Broken){return "损毁"}else{return "正常"} })()
    ...
    if(Command == "列表"){
        ...
        UserData.Units.forEach((v,i)=>{
            Ret = Ret + "\r\n" + i + "." + UserData.Language.Get(v.Root.Name) + (()=>{ if(v.Broken){return "[阵亡]"}else{return ""} })();
        });
